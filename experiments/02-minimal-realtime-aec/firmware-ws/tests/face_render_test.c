...
int main(void)
{
    ...
    unsigned int family_counts[FACE_RENDER_FAMILY_TOON + 1U] = {0};
    ...
    for (size_t raw_profile = 0;
        ...
        assert(info.family <= FACE_RENDER_FAMILY_TOON);
    ...
    for (size_t family = 0; family < FACE_RENDER_FAMILY_TOON; ++family) {
        assert(family_counts[family] >= 6U);
    }
    /*
     * Replacement modules can promote legacy robot/pixel slots into complete
     * close-up acting rigs, so the toon family is intentionally extensible.
     */
    assert(family_counts[FACE_RENDER_FAMILY_TOON] >= 3U);
