...
output_dir=${1:-/tmp/stackchan-face-eye-actors}
build_dir=$(mktemp -d "${TMPDIR:-/tmp}/face-eye-actors-build.XXXXXX")
...
clang $common_flags -O1 -g \
    ...
    "$script_dir/face_eye_actors_test.c" \
    -o "$build_dir/face_eye_actors_test"

ASAN_OPTIONS=halt_on_error=1 \
UBSAN_OPTIONS=halt_on_error=1 \
    "$build_dir/face_eye_actors_test" \
...
clang $common_flags -O2 \
    -I"$main_dir" \
    "$main_dir/face_eye_actors.c" \
    "$script_dir/face_eye_actors_dump.c" \
    -o "$build_dir/face_eye_actors_dump"
"$build_dir/face_eye_actors_dump" "$output_dir"

clang $common_flags -O3 \
    -I"$main_dir" \
    "$main_dir/face_eye_actors.c" \
    "$script_dir/face_eye_actors_bench.c" \
    -o "$build_dir/face_eye_actors_bench"
benchmark_result=$("$build_dir/face_eye_actors_bench")
printf '%s\n' "$benchmark_result" >"$output_dir/benchmark.txt"
printf '%s\n' "$benchmark_result"

clang $common_flags -Os -ffunction-sections -fdata-sections \
    -I"$main_dir" \
    -c "$main_dir/face_eye_actors.c" \
    -o "$build_dir/face_eye_actors.o"

if nm -u "$build_dir/face_eye_actors.o" |
...

{
    ...
    wc -c "$build_dir/face_eye_actors.o"
    echo
    echo "Platform size breakdown:"
    size "$build_dir/face_eye_actors.o"
    echo
    echo "Undefined symbols (no heap allocator references permitted):"
    nm -u "$build_dir/face_eye_actors.o"
