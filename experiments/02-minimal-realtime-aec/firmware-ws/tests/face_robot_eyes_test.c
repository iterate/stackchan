...
static face_stage_cue_t full_expression_cue(uint8_t expression)
{
...
static void test_stage_cues_preserve_bilateral_eye_mass(void)
{
    const uint32_t fixed_clock = 0U;
    for (size_t raw = 0U; raw < face_robot_eyes_profile_count(); ++raw) {
        const face_robot_eyes_profile_t profile =
            (face_robot_eyes_profile_t)raw;
        for (uint8_t expression = FACE_EXPRESSION_NEUTRAL;
            ...
            const face_stage_cue_t cue =
                full_expression_cue(expression);
            ...
            const int32_t smaller =
                pose.openness_q8[0] < pose.openness_q8[1]
                    ? pose.openness_q8[0]
                    : pose.openness_q8[1];
            const int32_t larger =
                pose.openness_q8[0] > pose.openness_q8[1]
                    ...
                    : pose.openness_q8[1];
            /*
             * Authored emotions may squint or close both eyes, but they
             * must never delete just one eye and masquerade as clipping.
             * Actual procedural blinks are tested separately over time.
             */
            assert(smaller >= 36);
...
int main(void)
{
    ...
    test_stage_cues_preserve_bilateral_eye_mass();
