...
enum {
    CT_FRAMES_PER_STYLE = 360,
};

static uint64_t ct_now_ns(void)
{
...
static face_render_key_t ct_benchmark_key(void)
{
...
int main(void)
{
    ...
    face_render_key_t key = ct_benchmark_key();
    uint32_t checksum = 2166136261U;
    const uint64_t start = ct_now_ns();
    for (size_t style = 0U;
         style < FACE_CLOSEUP_TOON_COUNT;
         ++style) {
        for (uint32_t frame = 0U;
             frame < CT_FRAMES_PER_STYLE;
             ++frame) {
            key.viseme = (uint8_t)(frame % FACE_VISEME_COUNT);
            key.viseme_secondary =
                (uint8_t)((frame + 1U) % FACE_VISEME_COUNT);
            key.viseme_blend = (uint8_t)(frame * 29U);
            key.audio_level = (uint8_t)(62U + frame % 180U);
            key.stage_expression =
                (uint8_t)(frame % FACE_EXPRESSION_COUNT);
            key.controls.look_x =
                (int8_t)((int)(frame % 31U) - 15);
            key.controls.look_y =
                (int8_t)((int)((frame * 3U) % 25U) - 12);
            key.controls.brow =
                (int8_t)((int)((frame * 5U) % 25U) - 12);
            key.speech_phase =
                (uint8_t)(frame % (FACE_SPEECH_ENDING + 1U));
            ...
            checksum ^= pixels[
                (frame * 7919U) % FACE_CLOSEUP_TOON_PIXEL_COUNT];
            checksum *= 16777619U;
        }
    }
    const uint64_t elapsed = ct_now_ns() - start;
    const uint64_t rendered =
        (uint64_t)FACE_CLOSEUP_TOON_COUNT * CT_FRAMES_PER_STYLE;
