...
enum {
    RR_FRAMES_PER_STYLE = 360,
};

static uint64_t rr_now_ns(void)
{
...
static face_render_key_t rr_benchmark_key(void)
{
...
int main(void)
{
    ...
    face_render_key_t key = rr_benchmark_key();
    uint32_t checksum = 2166136261U;
    const uint64_t start = rr_now_ns();
    for (size_t style = 0U; style < FACE_ROBOT_REDUX_COUNT; ++style) {
        for (uint32_t frame = 0U;
             frame < RR_FRAMES_PER_STYLE;
             ++frame) {
            ...
            checksum ^= pixels[
                (frame * 7919U) % FACE_ROBOT_REDUX_PIXEL_COUNT];
            checksum *= 16777619U;
        }
    }
    const uint64_t elapsed = rr_now_ns() - start;
    const uint64_t rendered =
        (uint64_t)FACE_ROBOT_REDUX_COUNT * RR_FRAMES_PER_STYLE;
