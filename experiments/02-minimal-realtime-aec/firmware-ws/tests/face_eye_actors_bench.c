...
int main(void)
{
    ...
    for (size_t style = 0U; style < FACE_EYE_ACTOR_COUNT; ++style) {
        for (uint32_t frame = 0U; frame < FRAMES_PER_STYLE; ++frame) {
    ...
    const uint64_t rendered =
        (uint64_t)FACE_EYE_ACTOR_COUNT * FRAMES_PER_STYLE;
    ...
    printf("actors=%d frames=%llu elapsed_ns=%llu ns_per_frame=%llu "
