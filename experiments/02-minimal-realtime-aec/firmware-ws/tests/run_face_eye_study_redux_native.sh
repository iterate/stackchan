...
artifact_directory="${script_directory}/artifacts/face_eye_study_redux"
build_directory="${TMPDIR:-/tmp}/stackchan-face-eye-study-redux"
...
sources=("${main_directory}/face_eye_study_redux.c")

"${compiler}" "${common_flags[@]}" \
    "${sources[@]}" \
    "${script_directory}/face_eye_study_redux_test.c" \
...
"${compiler}" "${common_flags[@]}" \
    "${sources[@]}" \
    "${script_directory}/face_eye_study_redux_dump.c" \
    -o "${build_directory}/dump"
"${build_directory}/dump" "${artifact_directory}"

"${compiler}" "${common_flags[@]}" \
    "${sources[@]}" \
    "${script_directory}/face_eye_study_redux_benchmark.c" \
...
printf '%s\n' \
    "face_eye_study_redux: native, ASan, UBSan, dump, and benchmark passed"
