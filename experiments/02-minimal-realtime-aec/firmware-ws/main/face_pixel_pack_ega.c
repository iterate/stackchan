    ...
    face_pixel_pack_landmarks_t *landmarks)
{
    ...
    const int left_corner_y =
        mouth_y - pose->mouth_corner_left / 24;
    const int right_corner_y =
        mouth_y - pose->mouth_corner_right / 24;
    fpp_line(
    ...
    if (pose->cheek > 24) {
        const uint8_t cheek_colour =
            pose->cheek > 80 ? EGA_LIGHT_MAGENTA : EGA_RED;
        fpp_fill_checker(
            surface, head_x - 12, head_y + 5, 4, 3,
            EGA_LIGHT_RED, cheek_colour, 2U);
        fpp_fill_checker(
            surface, head_x + 9, head_y + 5, 4, 3,
            EGA_LIGHT_RED, cheek_colour, 2U);
    }
    fpp_fill_checker(
