...
/*
 * Pixel portraits need the same parented eye hierarchy as the smooth robot
 * rigs.  Keep the sclera fixed, crop the opening about its centre, and clamp
 * the pupil inside the visible aperture.  Brows are anchored to the eye
 * socket rather than allowed to wander into hair or outside the portrait.
 */
    ...
    uint16_t brow_color)
{
    ...
    for (size_t eye = 0U; eye < 2U; ++eye) {
        const uint8_t opening = eye_open(requested[eye], motion);
        int32_t visible_height =
            max_i32(grid, full_height * opening / 255);
        visible_height =
            max_i32(grid, (visible_height / grid) * grid);
        visible_height = min_i32(full_height, visible_height);
        const int32_t left = centers[eye] - eye_width / 2;
        const int32_t top = center_y - visible_height / 2;
        fill_rect(
            canvas, left - grid, top - grid,
            eye_width + grid * 2, visible_height + grid * 2,
            outline);
        fill_rect(
            canvas, left, top, eye_width, visible_height, sclera);

        if (opening > 34U && visible_height >= grid * 2) {
            const int32_t pupil_width =
                max_i32(grid * 2, eye_width / 4);
            const int32_t pupil_height =
                max_i32(grid, visible_height - grid * 2);
            const int32_t gaze_x = clamp_i16(
                motion->gaze_x / 2,
                -eye_width / 2 + pupil_width / 2 + grid,
                eye_width / 2 - pupil_width / 2 - grid);
            const int32_t gaze_y = clamp_i16(
                motion->gaze_y / 3,
                -visible_height / 2 + pupil_height / 2 + grid,
                visible_height / 2 - pupil_height / 2 - grid);
            fill_rect(
                canvas,
        ...
        const int32_t outer_action = clamp_i16(
        ...
        if (eye == 0U) {
            draw_line(
                canvas, left, outer_y,
                left + eye_width, inner_y,
                grid, brow_color);
        } else {
            draw_line(
                canvas, left, inner_y,
                left + eye_width, outer_y,
                grid, brow_color);
        }
    }
}
...
/*
 * A bounded six-point mouth gives pixel portraits continuous jaw/width
 * articulation without the detached red rectangles produced by the old
 * sprite overlay.  It deliberately remains chunky and integer-only.
 */
...
{
    const face_keyframe_t *keyframe = &render_key->controls;
    const int32_t stage_weight = render_key->expression_weight;
    int32_t width_offset = 0;
    int32_t height_offset = 0;
    int32_t left_corner_offset = 0;
    int32_t right_corner_offset = 0;
    bool authored_grit = false;
    switch (render_key->stage_expression) {
    case FACE_EXPRESSION_WARM:
        width_offset = 2;
        left_corner_offset = 2;
        right_corner_offset = 2;
        break;
    case FACE_EXPRESSION_JOY:
        width_offset = 5;
        height_offset = 1;
        left_corner_offset = 4;
        right_corner_offset = 4;
        break;
    case FACE_EXPRESSION_CONCERN:
        width_offset = -2;
        left_corner_offset = -3;
        right_corner_offset = -3;
        break;
    case FACE_EXPRESSION_SURPRISE:
        width_offset = -6;
        height_offset = 5;
    ...
    width_offset = width_offset * stage_weight / 255;
    height_offset = height_offset * stage_weight / 255;
    left_corner_offset =
        left_corner_offset * stage_weight / 255;
    right_corner_offset =
        right_corner_offset * stage_weight / 255;

    const int32_t half_width = clamp_i16(
        7 + keyframe->mouth_width / 10 -
            keyframe->mouth_round / 24 + width_offset,
        7, max_half_width);
    const int32_t half_height = clamp_i16(
        2 + keyframe->mouth_open / 18 +
            keyframe->mouth_round / 64 + height_offset,
        2, max_half_height);
    const int32_t left_lift = clamp_i16(
        render_key->mouth_corner_left / 14 +
            left_corner_offset,
        -7, 7);
    const int32_t right_lift = clamp_i16(
        render_key->mouth_corner_right / 14 +
            right_corner_offset,
        -7, 7);
    const bool closed =
        ...
        (authored_grit && stage_weight > 176 &&
         keyframe->mouth_open < 154U);
    if (closed) {
    ...
    const motion_t *motion)
{
    const face_keyframe_t *keyframe = &render_key->controls;
    /*
     * Pixel art should move in whole pixels, but a two-pixel multiplier made
     * large portraits visibly judder on ordinary PCM envelope changes.
     */
    ...
    case FACE_RENDER_VGA_ELDER: {
        ...
        fill_polygon(
            ...
            5U, skin_light);
        draw_pixel_mouth_rig(
            canvas, render_key, 80, 82 + bob, 21, 10,
            RGB565(132, 60, 68), RGB565(44, 16, 24),
    ...
    case FACE_RENDER_TALKIE_CLOSEUP: {
        ...
        fill_polygon(
            ...
            4U, shadow);
        draw_pixel_mouth_rig(
            canvas, render_key, 82, 80 + bob, 23, 11,
    ...
    case FACE_RENDER_POCKET_RPG: {
        ...
        draw_pixel_mouth_rig(
            canvas, render_key, 80, 78 + bob, 20, 10,
    ...
    case FACE_RENDER_DITHERED_ROGUE: {
        ...
        draw_pixel_mouth_rig(
            canvas, render_key, 80, 81 + bob, 21, 10,
    ...
    const motion_t *motion)
{
    ...
    if (glitch) {
         ...
         */
        fill_ellipse(canvas, 80, 60, 43, 49, dark);
        fill_ellipse_blend(
            canvas, 80, 60, 39, 45,
    ...
    draw_emissive_robot_eye(
        canvas, render_key, motion, true,
        57, eye_y, glitch ? 31 : 32, glitch ? 25 : 27,
        glitch ? 4 : 10, eye_color, dark, true);
    draw_emissive_robot_eye(
        canvas, render_key, motion, false,
        103, eye_y, glitch ? 31 : 32, glitch ? 25 : 27,
        glitch ? 4 : 10, eye_color, dark, true);
    draw_brows(
        canvas, 57, 103, 31,
        glitch ? 13 : 14, motion, accent, true);
    draw_robot_mouth_rig(
    ...
    uint32_t sample_clock)
{
    ...
    fill_ellipse_blend(canvas, 80, 59, 34, 32, dark, 172U);
    draw_basic_eyes(
        canvas, 65, 95, 52, 9, 7, motion,
        keyframe->eye_left_open, keyframe->eye_right_open,
        cyan, violet, dark);
    draw_brows(
        canvas, 65, 95, 37, 10, motion,
        blend565(cyan, violet, 112U), true);
    draw_robot_mouth_rig(
        canvas, render_key, 80, 77,
        blend565(cyan, violet, 148U), dark,
        RGB565(248, 252, 255), RGB565(255, 102, 176));
}
    ...
    uint32_t sample_clock)
{
    ...
    const uint8_t eye_opening = (uint8_t)(
        ...
        2U);
    const int32_t eye_height =
        max_i32(2, 13 * eye_opening / 255);
    const int32_t gaze_x = clamp_i16(motion->gaze_x, -8, 8);
    const int32_t gaze_y =
        clamp_i16(motion->gaze_y, -eye_height / 2, eye_height / 2);
    ...
    const int32_t brow_y =
        34 - motion->brow / 3;
    const int32_t brow_tilt = clamp_i16(
        ((int32_t)motion->brow_outer_right -
         motion->brow_outer_left) /
            24,
        -6, 6);
    draw_line(
        canvas, 58, brow_y - brow_tilt,
        102, brow_y + brow_tilt, 4, red);
    draw_robot_mouth_rig(
    ...
    uint32_t sample_clock)
{
    ...
    const uint8_t energy =
        (uint8_t)max_i32(render_key->audio_level, keyframe->mouth_open);
    const int32_t left_eye_height = max_i32(
        1, 6 * eye_open(keyframe->eye_left_open, motion) / 255);
    const int32_t right_eye_height = max_i32(
        1, 6 * eye_open(keyframe->eye_right_open, motion) / 255);
    canvas_clear(canvas, RGB565(0, 0, 0));
    for (int32_t y = 0; y < 48; ++y) {
        for (int32_t x = 0; x < 64; ++x) {
            const int32_t left = ellipse_distance_q8(
                x, y, 20 + motion->gaze_x / 3,
                19 + motion->gaze_y / 3, 9, left_eye_height);
            const int32_t right = ellipse_distance_q8(
                x, y, 44 + motion->gaze_x / 3,
                19 + motion->gaze_y / 3, 9, right_eye_height);
            const int32_t mouth_x = x * 80 / 64;
            const int32_t mouth_y = y * 60 / 48;
            const int32_t mouth =
                mouth_distance_q8(
                    mouth_x, mouth_y, keyframe, motion);
            const int32_t distance =
                min_i32(min_i32(left, right), mouth);
            uint8_t glow = glow_from_distance(distance, energy);
            glow = glow > 180U ? 255U
                               : glow > 90U ? 150U
                                            : glow > 30U ? 70U : 0U;
            const uint16_t color = cyber_palette(
                (uint8_t)(x * 5 + y * 7 + frame * 5U),
                glow, 1U);
            const int32_t x0 = x * FACE_RENDER_WIDTH / 64;
            const int32_t x1 = (x + 1) * FACE_RENDER_WIDTH / 64;
            const int32_t y0 = y * FACE_RENDER_HEIGHT / 48;
            const int32_t y1 = (y + 1) * FACE_RENDER_HEIGHT / 48;
            fill_rect(
                canvas, x0, y0,
                max_i32(1, x1 - x0 - 1),
                max_i32(1, y1 - y0 - 1), color);
        }
    }
    const uint16_t led = RGB565(72, 255, 166);
    const int32_t brow_base = 24 - motion->brow / 4;
    for (int32_t eye = 0; eye < 2; ++eye) {
        const int32_t center = eye == 0 ? 50 : 110;
        const int32_t outer =
            eye == 0 ? motion->brow_outer_left
                     : motion->brow_outer_right;
        const int32_t slope = clamp_i16(
            (motion->brow_inner - outer) / 24, -4, 4);
        for (int32_t segment = -2; segment <= 2; ++segment) {
            const int32_t direction = eye == 0 ? segment : -segment;
            fill_rect(
                canvas,
                center + segment * 6 - 2,
                brow_base + direction * slope / 2,
                4, 3, led);
        }
    }
    if (render_key->cheek > 24U) {
        ...
        for (int32_t segment = 0; segment < cheek_segments; ++segment) {
            fill_rect(canvas, 25 + segment * 6, 82, 4, 3, led);
            fill_rect(canvas, 131 - segment * 6, 82, 4, 3, led);
        }
    }
}
    ...
    uint32_t sample_clock)
{
    ...
    fill_ellipse_blend(canvas, 80, 58, 34, 31, dark, 210U);
    draw_basic_eyes(
        canvas, 64, 96, 49, 11, 8, motion,
        keyframe->eye_left_open, keyframe->eye_right_open,
        cyan, rose, dark);
    draw_brows(canvas, 64, 96, 34, 11, motion, cyan, true);
    draw_robot_mouth_rig(
        canvas, render_key, 80, 78,
        rose, dark, RGB565(252, 248, 255), RGB565(255, 118, 182));
}
    ...
    uint32_t sample_clock)
{
    ...
    const int32_t wobble = motion->tilt;
    const point_t mask[] = {
        {80, 10 + motion->bob},
        {132 + wobble, 38},
        {122 + wobble, 91},
    ...
    const int32_t eye_height =
        max_i32(1, 10 * eye_open(
            keyframe->eye_left_open, motion) / 255);
    draw_line(
        canvas, 42 + motion->gaze_x, 51 + motion->gaze_y,
        69 + motion->gaze_x,
        51 + motion->gaze_y + eye_height / 3, 3, cyan);
    draw_line(
        canvas, 91 + motion->gaze_x,
        51 + motion->gaze_y + eye_height / 3,
        118 + motion->gaze_x, 51 + motion->gaze_y, 3, cyan);
    draw_brows(
        canvas, 56, 104, 36, 13, motion,
        blend565(cyan, RGB565(234, 60, 210), 82U), true);
    draw_polygon_mouth(
        canvas, keyframe, 80, 78, 1,
        blend565(cyan, RGB565(234, 60, 210),
                 (uint8_t)(frame * 7U)),
        RGB565(2, 12, 18));
}
...
{
    return profile_is_valid(profile) ? PROFILES[profile].slug : NULL;
}

const char *face_render_profile_name(face_render_profile_t profile)
{
    return profile_is_valid(profile) ? PROFILES[profile].name : NULL;
}
