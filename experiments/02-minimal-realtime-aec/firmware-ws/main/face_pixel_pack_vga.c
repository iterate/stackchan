    ...
    face_pixel_pack_landmarks_t *landmarks)
{
    ...
    if (pose->cheek > 50) {
        const uint8_t colour =
            pose->cheek > 100 ? VGA_LIP_MID : VGA_SKIN_SHADOW;
        fpp_fill_checker(
            surface, centre_x - 22, face_y + 8,
            8, 5, VGA_SKIN_MID, colour, 1U);
        fpp_fill_checker(
            surface, centre_x + 15, face_y + 8,
            8, 5, VGA_SKIN_SHADOW, colour, 1U);
    }
