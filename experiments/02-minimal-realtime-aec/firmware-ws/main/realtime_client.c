...
static StaticEventGroup_t s_control_events_storage;
...
static int64_t s_input_journal_started_us;
static uint32_t s_input_journal_frames;
static uint32_t s_input_journal_bytes;
static uint32_t s_input_journal_samples;
...
static void journal_outbound_text(const char *payload, size_t length);
...
static void note_error(const char *message)
{
    ...
    realtime_event_log_record(
        "sys", "error", "realtime.error", message, 0, 0);
...
static int send_text(const char *text)
{
    ...
    journal_outbound_text(text, (size_t)length);
    return sent;
}
...
static void request_reconnect(const char *reason)
{
    ...
    app_status_set_phase(
        ...
        reason != NULL ? reason :
                         ...
                         " session restarting");
    if (!diagnostics_blocks_realtime()) {
        ui_set_face_only(false);
        ui_set_status("Reconnecting", "Starting a fresh voice session...");
        ui_set_button_label("Please wait");
    }
    xEventGroupSetBits(s_control_events, CONTROL_RECONNECT);
}

static void restart_websocket(void)
{
    if (s_client == NULL) {
        return;
    }
    ...
    /*
     * A provider-initiated RFC6455 close is clean, so the WebSocket component
     * exits instead of applying its transport-error auto-reconnect. Stop/wait
     * here, outside its event task, then start the retained client again.
     */
    (void)esp_websocket_client_stop(s_client);
    for (unsigned attempt = 1; attempt <= 3; ++attempt) {
        __atomic_fetch_add(
            &s_reconnect_attempts, 1, __ATOMIC_RELAXED);
        const esp_err_t result = esp_websocket_client_start(s_client);
        if (result == ESP_OK) {
            ESP_LOGI(
                TAG, "Fresh Realtime WebSocket session requested "
                     "(attempt %u)",
                attempt);
            return;
        }
        ESP_LOGW(
            TAG, "Realtime restart attempt %u failed: %s",
            attempt, esp_err_to_name(result));
        vTaskDelay(pdMS_TO_TICKS(500));
    }
    note_error("Unable to restart Realtime WebSocket");
}

static void journal_input_audio(size_t payload_bytes, size_t samples)
{
    const int64_t now = esp_timer_get_time();
    if (s_input_journal_started_us == 0) {
        s_input_journal_started_us = now;
    }
    ++s_input_journal_frames;
    s_input_journal_bytes +=
        payload_bytes > UINT32_MAX - s_input_journal_bytes
            ? UINT32_MAX - s_input_journal_bytes
            : (uint32_t)payload_bytes;
    s_input_journal_samples +=
        samples > UINT32_MAX - s_input_journal_samples
            ? UINT32_MAX - s_input_journal_samples
            : (uint32_t)samples;
    if (now - s_input_journal_started_us < 1000000) {
        return;
    }

    char summary[96];
    snprintf(
        summary, sizeof(summary), "frames=%u interval_ms=%lld",
        (unsigned)s_input_journal_frames,
        (long long)((now - s_input_journal_started_us) / 1000));
    realtime_event_log_record(
        "tx", "audio_summary", "input_audio", summary,
        s_input_journal_bytes, s_input_journal_samples);
    s_input_journal_started_us = now;
    s_input_journal_frames = 0;
    s_input_journal_bytes = 0;
    s_input_journal_samples = 0;
}
...
static bool send_input_audio(const int16_t *samples, size_t sample_count)
{
    ...
    if (STACKCHAN_REALTIME_BINARY_TRANSPORT) {
        ...
        if (sent != (int)pcm_bytes) {
            ...
            request_reconnect(
                STACKCHAN_REALTIME_PROVIDER_DISPLAY
                " microphone upload failed; restarting");
            return false;
        }
        __atomic_fetch_add(
        ...
        __atomic_fetch_add(
            ...
            __ATOMIC_RELAXED);
        journal_input_audio(pcm_bytes, transport_sample_count);
        return true;
    }
    ...
    __atomic_fetch_add(
        &s_uploaded_input_samples, transport_sample_count,
        __ATOMIC_RELAXED);
    journal_input_audio(message_bytes, transport_sample_count);
    return true;
}
...
static void control_task(void *argument)
{
    (void)argument;
    while (true) {
        const EventBits_t commands = xEventGroupWaitBits(
            s_control_events,
            CONTROL_CONFIGURE | CONTROL_CANCEL | CONTROL_OUTPUT_PROBE |
                CONTROL_RECONNECT,
            pdTRUE, pdFALSE, pdMS_TO_TICKS(20));

        if ((commands & CONTROL_RECONNECT) != 0) {
            restart_websocket();
        }
        ...
        if (atomic_bool(&s_streaming) &&
            ...
            const size_t samples =
                read_clean_chunk(
                    pdMS_TO_TICKS(40), pdMS_TO_TICKS(40));
            if (samples > 0) {
                speech_leveler_metrics_t uplink_metrics;
                speech_leveler_process(
                    s_input_device, samples, STACKCHAN_UPLINK_GAIN_DB,
                    &uplink_metrics);
                if (!send_input_audio(s_input_device, samples)) {
...
static const char *journal_event_text(const cJSON *root, const char *type)
{
    if (type == NULL) {
        return NULL;
    }
    if (strstr(type, "transcript") != NULL ||
        strstr(type, "transcription") != NULL) {
        const char *transcript = json_string(root, "transcript");
        return transcript != NULL ? transcript : json_string(root, "delta");
    }
    if (strcmp(type, "error") == 0) {
        const cJSON *error =
            cJSON_GetObjectItemCaseSensitive(root, "error");
        return cJSON_IsObject(error) ? json_string(error, "message") : NULL;
    }
    if (strcmp(type, "conversation.item.create") == 0) {
        const cJSON *item =
            cJSON_GetObjectItemCaseSensitive(root, "item");
        const cJSON *content = cJSON_IsObject(item)
                                   ? cJSON_GetObjectItemCaseSensitive(
                                         item, "content")
                                   : NULL;
        const cJSON *first =
            cJSON_IsArray(content) ? cJSON_GetArrayItem(content, 0) : NULL;
        return cJSON_IsObject(first) ? json_string(first, "text") : NULL;
    }
    return NULL;
}

static size_t base64_audio_samples(const char *base64)
{
    if (base64 == NULL) {
        return 0;
    }
    const size_t encoded = strlen(base64);
    size_t decoded = encoded / 4 * 3;
    if (encoded > 0 && base64[encoded - 1] == '=') {
        --decoded;
    }
    if (encoded > 1 && base64[encoded - 2] == '=') {
        --decoded;
    }
    return decoded / sizeof(int16_t);
}
...
static void journal_outbound_text(const char *payload, size_t length)
{
    ...
    if (root == NULL) {
        realtime_event_log_record(
            "tx", "json", "unparsed", NULL, length, 0);
        return;
    }
    const char *type = json_string(root, "type");
    realtime_event_log_record(
        "tx", "json", type != NULL ? type : "unknown",
        journal_event_text(root, type), length, 0);
    cJSON_Delete(root);
}

static void journal_server_event(const cJSON *root, const char *type,
                                 size_t payload_bytes)
{
    const bool audio_json =
        type != NULL && strcmp(type, "response.output_audio.delta") == 0;
    const char *base64 = audio_json ? json_string(root, "delta") : NULL;
    realtime_event_log_record(
        "rx", audio_json ? "audio_json" : "json",
        type != NULL ? type : "unknown",
        audio_json ? NULL : journal_event_text(root, type),
        payload_bytes, base64_audio_samples(base64));
}
...
static void process_server_event(const char *payload, size_t length)
{
    ...
    journal_server_event(root, type, length);
    ...
    if (strcmp(type, "session.updated") == 0) {
        ...
        const face_stream_event_t face_event = {
            .type = FACE_STREAM_USER_SPEECH_STARTED,
        };
        audio_pipeline_face_event(&face_event);
        ...
        if (atomic_bool(&s_response_active) ||
            ...
            atomic_set_bool(&s_accept_output, false);
            request_output_flush();
            xEventGroupSetBits(s_control_events, CONTROL_CANCEL);
            ESP_LOGI(
                TAG,
                "Server VAD detected barge-in; response cancellation "
                "requested");
        } else {
        ...
        if (atomic_bool(&s_vad_speech_active)) {
            ...
            ESP_LOGW(
                TAG,
                "Response created during live speech; cancellation requested");
        } else {
        ...
        copy_state_string(
            ...
            json_string(root, "transcript"));
        ESP_LOGI(
            TAG, "Input transcript: %.180s", s_input_transcript);
    } else if (strcmp(type, "response.done") == 0) {
        const face_stream_event_t face_event = {
            .type = FACE_STREAM_ASSISTANT_RESPONSE_DONE,
        };
        audio_pipeline_face_event(&face_event);
        atomic_set_bool(&s_generation_active, false);
        atomic_set_bool(&s_accept_output, false);
        ...
        ESP_LOGI(
            TAG, "Output transcript: %.180s", s_output_transcript);
        ...
        if (message != NULL &&
            (strstr(message, "timed out") != NULL ||
             strstr(message, "session expired") != NULL)) {
            request_reconnect(
                STACKCHAN_REALTIME_PROVIDER_DISPLAY
...
static void accept_websocket_data(const esp_websocket_event_data_t *data)
{
    ...
    if (offset + chunk == total && data->fin) {
        if (s_message_opcode == 0x2) {
            ...
            realtime_event_log_record(
                "rx", "audio_binary", "response.output_audio",
                NULL, total, total / sizeof(int16_t));
    ...
    void *event_data)
{
    ...
    case WEBSOCKET_EVENT_CONNECTED:
        ...
        realtime_event_log_record(
            "sys", "connection", "websocket.connected", NULL, 0, 0);
        ui_set_face_only(false);
        xEventGroupSetBits(s_control_events, CONTROL_CONFIGURE);
    ...
    case WEBSOCKET_EVENT_CLOSED:
        ...
        request_output_flush();
        realtime_event_log_record(
            "sys", "connection", "websocket.disconnected", NULL, 0, 0);
        app_status_set_phase(
            APP_PHASE_CONNECTING,
            STACKCHAN_REALTIME_PROVIDER_DISPLAY
        ...
        if (event_id == WEBSOCKET_EVENT_CLOSED) {
            request_reconnect(
                STACKCHAN_REALTIME_PROVIDER_DISPLAY
                " closed the session; reconnecting");
        }
        break;
...
esp_err_t realtime_client_init(void)
{
    ...
    s_control_events =
        xEventGroupCreateStatic(&s_control_events_storage);
    if (s_control_events == NULL) {
        ESP_LOGE(TAG, "Unable to create Realtime control event group");
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t realtime_client_reconnect(void)
{
    if (!s_initialized || s_control_events == NULL || s_client == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    request_reconnect(
        STACKCHAN_REALTIME_PROVIDER_DISPLAY
...
esp_err_t realtime_client_run_output_probe(void)
{
    ...
    xEventGroupSetBits(s_control_events, CONTROL_OUTPUT_PROBE);
    return ESP_OK;
}
...
void realtime_client_snapshot(realtime_client_snapshot_t *snapshot)
{
    ...
    snapshot->reconnect_attempts =
        __atomic_load_n(&s_reconnect_attempts, __ATOMIC_RELAXED);
