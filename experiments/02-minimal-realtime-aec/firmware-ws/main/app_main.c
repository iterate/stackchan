...
static esp_err_t avatar_next(void *context)
{
    (void)context;
    return ui_next_avatar();
}

static void screen_tapped(void)
{
    if (!face_avatar_registry_next()) {
        ESP_LOGE(TAG, "Unable to select next sprite avatar");
    }
}
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(realtime_event_log_init());

    /*
     * Configure the StackChan body MCU before LVGL starts polling touch on
     * the shared CoreS3 I2C bus. Runtime transactions are serialized by the
     * IDF bus driver, but early init is deliberately uncontended.
     */
