 ...
 * The physiological down-phase is resampled for the 30 fps display rather
 * than copied literally: 75 ms would leave only two visible poses and turn
 * these large solid eyes into a hard cut. Four display-aware intermediate
 * poses close the lid, followed by a short full-closure hold and a longer
 * reopening, with a small cartoon overshoot when `bounce` is set.
...
{
    const int32_t close_ms = 120;
    const int32_t hold_ms = 16;
    if (u_ms < 0) {
        return 256;
    }
    if (u_ms < close_ms) {
        const int32_t linear = (u_ms * 256) / close_ms;
        /*
         * Half linear, half smoothstep caps peak velocity at 1.25x while
         * retaining eased endpoints. A pure smoothstep needs a much longer
         * blink to expose the same number of useful 30 fps silhouettes.
         */
        int32_t e = (linear + fre_smoothstep_q8(linear)) / 2;
    ...
    if (v < reopen_ms) {
        /*
         * A cubic ease-out spends almost half the aperture in its first
         * 30 fps sample. The same velocity-bounded soft step as closure
         * exposes the intermediate silhouettes instead.
         */
        const int32_t linear = (v * 256) / reopen_ms;
        int32_t e = (linear + fre_smoothstep_q8(linear)) / 2;
    ...
    fre_blink_state_t *out)
{
    ...
    /*
     * Profiles keep their relative fast/slow character via reopen_pct.
     * The bounded easing below makes even Cozmo's 90% setting span several
     * useful display poses instead of collapsing to a cut.
     */
    ...
    fre_rig_t *rig)
{
    ...
    for (int eye = 0; eye < 2; ++eye) {
        rig->upper_lid_slope_q12[eye] = 0;
        rig->upper_lid_bend_q12[eye] = 0;
        rig->lower_lid_slope_q12[eye] = 0;
        rig->lower_lid_bend_q12[eye] = 0;
    }
