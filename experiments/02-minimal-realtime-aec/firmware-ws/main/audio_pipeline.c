...
#define CLEAN_BUFFER_BYTES \
    (STACKCHAN_AUDIO_SAMPLE_RATE * sizeof(int16_t) * 3)
#define PLAYBACK_STORAGE_BYTES (PLAYBACK_BUFFER_BYTES + 1)
#define CLEAN_STORAGE_BYTES (CLEAN_BUFFER_BYTES + 1)
#define STACKCHAN_AEC_MODE AEC_MODE_FD_LOW_COST
#define STACKCHAN_AEC_MODE_NAME "fd_low_cost"
...
static esp_err_t initialise_codecs(void)
{
    ...
    ESP_RETURN_ON_ERROR(
        bsp_audio_init(&i2s_config), TAG, "Initialize CoreS3 I2S");
    ...
    if (esp_codec_dev_set_vol_curve(s_speaker, &volume_curve) != ESP_OK ||
        ...
        esp_codec_dev_set_in_gain(
            s_microphone, s_microphone_gain_db) != ESP_OK ||
        esp_codec_dev_open(s_speaker, &format) != ESP_OK ||
        esp_codec_dev_open(s_microphone, &format) != ESP_OK) {
...
static void audio_task(void *argument)
{
    ...
    for (int frame = 0; frame < STACKCHAN_AUDIO_RX_PRIME_FRAMES; ++frame) {
        if (esp_codec_dev_read(
                s_microphone, s_raw_frame, (int)frame_bytes) != ESP_OK) {
    ...
    while (true) {
        ...
        if (diagnostics_is_running()) {
            diagnostics_fill_playback(s_reference_frame, s_frame_samples);
        } else {
            memset(s_reference_frame, 0, frame_bytes);
            const size_t received =
                xStreamBufferReceive(
                    s_playback, s_reference_frame, frame_bytes, 0);
            if (received > 0 && received < frame_bytes) {
                memset((uint8_t *)s_reference_frame + received, 0,
                       frame_bytes - received);
                app_status_note_playback_underrun();
            }
        }
        level_playback(s_reference_frame, s_frame_samples);

        const int64_t write_started_us = esp_timer_get_time();
        if (esp_codec_dev_write(
                s_speaker, s_reference_frame,
        ...
        /*
         * Drive the face from the exact levelled PCM accepted by the speaker,
         * on the audio task's playout clock. Network receipt and jitter-buffer
         * timing therefore cannot make the mouth lead the audible stream.
         */
        face_animator_push_pcm(
            &s_face_animator, s_reference_frame, s_frame_samples);
        if (esp_codec_dev_read(
                s_microphone, s_raw_frame,
                (int)frame_bytes) != ESP_OK) {
            app_status_note_audio_read_error();
            memset(s_raw_frame, 0, frame_bytes);
        }
        const int64_t read_finished_us = esp_timer_get_time();

        const int64_t reference_started_us = esp_timer_get_time();
        prepare_aec_reference(s_reference_frame, s_frame_samples);
        ...
        diagnostics_record_frame(
            s_raw_frame, s_reference_frame, s_clean_frame,
            s_frame_samples);
        audio_trace_record(
            s_raw_frame, s_reference_frame, s_clean_frame,
            s_frame_samples);
...
esp_err_t audio_pipeline_init(void)
{
    ...
    BaseType_t created = xTaskCreatePinnedToCore(
        audio_task, "aec_audio", STACKCHAN_AEC_TASK_STACK, NULL,
        STACKCHAN_AEC_TASK_PRIORITY, &s_audio_task,
...
esp_err_t audio_pipeline_set_microphone_gain(int gain_db)
{
    ...
    const esp_err_t result =
        esp_codec_dev_set_in_gain(s_microphone, (float)gain_db);
    if (result == ESP_OK) {
