...
static esp_err_t root_handler(httpd_req_t *request)
{
    static const char response[] =
        ...
        "\"realtime_events\":\"/api/debug/realtime-events.jsonl\","
        "\"recent_audio\":\"/api/debug/recent.wav\","
        "\"debug_state\":\"/api/debug/state\","
        "\"realtime\":\"/api/realtime\","
        "\"realtime_reconnect\":\"POST /api/realtime/reconnect\","
...
static esp_err_t avatar_status_handler(httpd_req_t *request)
{
    char response[256];
    snprintf(
        ...
        "\"name\":\"%s\""
        "}",
        (unsigned)face_avatar_registry_current_index(),
...
static esp_err_t avatar_next_handler(httpd_req_t *request)
{
    if (s_callbacks.next_avatar == NULL) {
        return send_error(
            request, "501 Not Implemented", "Avatar cycling unavailable");
    }
    const esp_err_t result =
        s_callbacks.next_avatar(s_callbacks.context);
    if (result != ESP_OK) {
        return send_error(
            request, "503 Service Unavailable", "Unable to select avatar");
    }
    return avatar_status_handler(request);
}
...
static esp_err_t status_handler(httpd_req_t *request)
{
    ...
    snprintf(
        ...
        (unsigned)timing.dma_tap_pairs,
        (unsigned)timing.dma_tap_tx_drops,
        (unsigned)timing.dma_tap_rx_drops,
        (unsigned)timing.dma_tap_sequence_skips,
...
static esp_err_t realtime_status_handler(httpd_req_t *request)
{
    ...
    cJSON_AddNumberToObject(
        root, "reconnect_attempts", snapshot.reconnect_attempts);
    cJSON_AddNumberToObject(
...
static bool append_bytes(char *destination, size_t capacity, size_t *used,
                         const char *source, size_t bytes)
{
    ...
    memcpy(destination + *used, source, bytes);
    *used += bytes;
    return true;
}

static bool append_literal(char *destination, size_t capacity, size_t *used,
                           const char *source)
{
    return append_bytes(
        destination, capacity, used, source, strlen(source));
}

static bool append_json_string(char *destination, size_t capacity,
                               size_t *used, const char *source)
{
    if (!append_literal(destination, capacity, used, "\"")) {
        return false;
    }
    const unsigned char *cursor =
        (const unsigned char *)(source != NULL ? source : "");
    while (*cursor != '\0') {
        ...
        if (escape != NULL) {
            if (!append_literal(destination, capacity, used, escape)) {
                return false;
            }
        } else if (*cursor < 0x20) {
            char unicode[7];
            ...
            if (bytes != 6 ||
                !append_bytes(
                    destination, capacity, used, unicode, (size_t)bytes)) {
                return false;
            }
        } else {
            const char value = (char)*cursor;
            if (!append_bytes(
                    destination, capacity, used, &value, 1)) {
                return false;
            }
        }
        ++cursor;
    }
    return append_literal(destination, capacity, used, "\"");
}

static uint32_t realtime_events_since(httpd_req_t *request)
{
    char query[48];
    char value[16];
    if (httpd_req_get_url_query_str(
            request, query, sizeof(query)) != ESP_OK ||
        httpd_query_key_value(
            query, "since", value, sizeof(value)) != ESP_OK) {
        return 0;
    }
    char *end = NULL;
    const unsigned long sequence = strtoul(value, &end, 10);
    return end != value && *end == '\0' && sequence <= UINT32_MAX
               ? (uint32_t)sequence
               : 0;
}

static size_t format_realtime_event_line(
    char *line, size_t capacity, const realtime_event_record_t *record)
{
    size_t used = 0;
    const int prefix = snprintf(
        line, capacity,
        "{\"sequence\":%u,\"timestamp_ms\":%llu,"
        "\"direction\":",
    ...
    if (!append_json_string(
            line, capacity, &used, record->direction) ||
        !append_literal(line, capacity, &used, ",\"kind\":") ||
        !append_json_string(line, capacity, &used, record->kind) ||
        !append_literal(line, capacity, &used, ",\"type\":") ||
        !append_json_string(line, capacity, &used, record->type) ||
        !append_literal(line, capacity, &used, ",\"payload_bytes\":")) {
        return 0;
    }
    const int counts = snprintf(
        line + used, capacity - used,
        "%u,\"audio_samples\":%u,\"text\":",
        (unsigned)record->payload_bytes,
        (unsigned)record->audio_samples);
    if (counts < 0 || (size_t)counts >= capacity - used) {
        return 0;
    }
    used += (size_t)counts;
    if (!append_json_string(line, capacity, &used, record->text) ||
        !append_literal(line, capacity, &used, "}\n")) {
        return 0;
    }
    return used;
}

static esp_err_t realtime_events_handler(httpd_req_t *request)
{
    realtime_event_record_t *records = heap_caps_malloc(
        STACKCHAN_REALTIME_EVENT_LOG_CAPACITY *
            sizeof(realtime_event_record_t),
        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    char *line = heap_caps_malloc(
        2048, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (records == NULL || line == NULL) {
        heap_caps_free(records);
        heap_caps_free(line);
        return send_error(request, "503 Service Unavailable",
                          "Not enough PSRAM to snapshot Realtime events");
    }

    realtime_event_log_info_t info;
    const size_t count = realtime_event_log_copy(
        records, STACKCHAN_REALTIME_EVENT_LOG_CAPACITY, &info);
    const uint32_t since = realtime_events_since(request);

    httpd_resp_set_type(request, "application/x-ndjson");
    httpd_resp_set_hdr(request, "Cache-Control", "no-store");
    httpd_resp_set_hdr(
        request, "Content-Disposition",
        "attachment; filename=\"realtime-events.jsonl\"");

    const int meta_bytes = snprintf(
        line, 2048,
        "{\"journal\":{\"capacity\":%u,\"available\":%u,"
        "\"newest_sequence\":%u,\"overwritten_records\":%u,"
        "\"lock_contention_drops\":%u,\"requested_since\":%u}}\n",
        (unsigned)info.capacity, (unsigned)info.available,
        (unsigned)info.newest_sequence,
        (unsigned)info.overwritten_records,
        (unsigned)info.lock_contention_drops, (unsigned)since);
    esp_err_t result =
        meta_bytes > 0 && meta_bytes < 2048
            ? httpd_resp_send_chunk(request, line, meta_bytes)
            : ESP_FAIL;
    for (size_t index = 0; index < count && result == ESP_OK; ++index) {
        if (records[index].sequence <= since) {
            continue;
        }
        const size_t bytes =
            format_realtime_event_line(line, 2048, &records[index]);
        result = bytes > 0
                     ? httpd_resp_send_chunk(
                           request, line, (ssize_t)bytes)
                     : ESP_FAIL;
    }
    if (result == ESP_OK) {
...
static esp_err_t realtime_reconnect_handler(httpd_req_t *request)
{
    const esp_err_t result = realtime_client_reconnect();
    if (result != ESP_OK) {
        return send_error(request, "409 Conflict",
                          "Realtime client is not initialized");
    }
    return send_json(
        request, "{\"ok\":true,\"requested\":\"fresh_session\"}");
}
...
static const httpd_uri_t s_routes[] = {
    ...
    {.uri = "/api/avatar",
     .method = HTTP_GET,
     .handler = avatar_status_handler},
    {.uri = "/api/avatar",
     .method = HTTP_POST,
     .handler = avatar_next_handler},
    ...
    {.uri = "/api/realtime/reconnect",
     .method = HTTP_POST,
     .handler = realtime_reconnect_handler},
    ...
    {.uri = "/api/debug/realtime-events.jsonl",
     .method = HTTP_GET,
     .handler = realtime_events_handler},
