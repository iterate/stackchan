    ...
    uint8_t palette_index)
{
    ...
    if (x0 >= FACE_RENDER_WIDTH || y0 >= FACE_RENDER_HEIGHT ||
    ...
    for (int32_t dy = 0; dy < scale; ++dy) {
        ...
        if (y < 0 || y >= FACE_RENDER_HEIGHT) {
            continue;
        }
        uint16_t *row = &blit->pixels[y * FACE_RENDER_WIDTH];
        for (int32_t dx = 0; dx < scale; ++dx) {
            const int32_t x = x0 + dx;
            if (x >= 0 && x < FACE_RENDER_WIDTH) {
                row[x] = color;
            }
        }
    }
    ...
    uint32_t sample_clock)
{
    ...
    const uint32_t triangle = phase < half
        ? phase
        : period - phase;
    return (int16_t)((triangle * 2U + half / 2U) / half);
}
    ...
    const face_sprite_cell_t *cell)
{
    if (cell->width == 0U || cell->height == 0U ||
        ...
        cell->height > FACE_RENDER_HEIGHT ||
...
static bool validate_atlas(const face_sprite_atlas_t *atlas)
{
    if (atlas->magic != FACE_SPRITE_MAGIC ||
        ...
        (uint32_t)atlas->native_width * atlas->scale >
            FACE_RENDER_WIDTH ||
    ...
    size_t pixel_capacity)
{
    if (player == NULL || player->atlas == NULL ||
        ...
        pixel_capacity < FACE_RENDER_PIXEL_COUNT) {
        return false;
    }
    ...
    const face_sprite_atlas_t *atlas = player->atlas;
    for (size_t pixel = 0U;
         pixel < FACE_RENDER_PIXEL_COUNT;
         ++pixel) {
        rgb565[pixel] = atlas->background;
    }
    ...
    size_t pixel_capacity)
{
    ...
    return face_sprite_render(
        &snapshot, render_key, sample_clock,
