...
from fspp.rig import (  # noqa: E402
...
class RasterTests(unittest.TestCase):
    ...
    def test_feature_only_composition_is_sparse(self) -> None:
        rig = replace(
            load_spec(FIXTURES / "bloomling_spec.json").rig,
            composition_mode="features_only",
            feature_regions=((1, 1, 3, 3),),
            transparent_colors=((20, 40, 20),),
            transparent_tolerance=0,
            blink="none",
        )
        image = png_io.Image.blank(6, 6)
        image.put(2, 2, (20, 40, 20, 255))  # keyed shell/skin
        image.put(3, 2, (240, 250, 90, 255))  # retained feature
        image.put(5, 5, (240, 250, 90, 255))  # outside feature region
        sparse = isolate_features(image, rig)
        self.assertEqual(sparse.get(2, 2)[3], 0)
        self.assertEqual(sparse.get(3, 2), (240, 250, 90, 255))
        self.assertEqual(sparse.get(5, 5)[3], 0)

    def test_feature_region_preserves_whole_connected_component(self) -> None:
        rig = replace(
            load_spec(FIXTURES / "bloomling_spec.json").rig,
            composition_mode="features_only",
            feature_regions=((1, 1, 2, 2),),
            transparent_colors=((20, 40, 20),),
            transparent_tolerance=0,
            blink="none",
        )
        image = png_io.Image.blank(8, 6)
        # The seed touches only the first two pixels, but the complete
        # diagonal/orthogonal eye stroke must survive outside the seed.
        for x, y in ((1, 1), (2, 2), (3, 2), (4, 2)):
            image.put(x, y, (240, 250, 90, 255))
        image.put(6, 4, (240, 250, 90, 255))  # unrelated baked mouth

        sparse = isolate_features(image, rig)

        self.assertEqual(sparse.get(4, 2), (240, 250, 90, 255))
        self.assertEqual(sparse.get(6, 4)[3], 0)

    def test_expression_brows_are_baked_only_for_declared_pose(self) -> None:
        spec = load_spec(FIXTURES / "bloomling_spec.json")
        from fspp.spec import BrowPoseSpec

        rig = replace(
            spec.rig,
            brow_color=(240, 250, 90),
            brow_thickness=1,
            brow_poses=(
                (
                    "neutral",
        ...
        image = png_io.Image.blank(12, 8)
        neutral = apply_expression_brows(image, rig, "neutral")
        joy = apply_expression_brows(image, rig, "joy")
        self.assertEqual(neutral.get(3, 2), (240, 250, 90, 255))
        self.assertEqual(neutral.get(8, 2), (240, 250, 90, 255))
        self.assertEqual(joy.get(3, 2)[3], 0)

    def test_feature_only_scale_registers_the_complete_face(self) -> None:
        spec = load_spec(FIXTURES / "bloomling_spec.json")
        rig = replace(
            spec.rig,
            canvas=(20, 12),
        ...
        image = png_io.Image.blank(40, 20)
        for x0 in (0, 30):
            for y in range(10):
                for x in range(x0, x0 + 10):
                    image.put(x, y, (240, 250, 90, 255))
        num, den, evidence = shared_scale({"neutral": image}, rig)
        canvas = normalize_portrait(image, rig, num, den)
        self.assertEqual(evidence["median_body"], [40, 10])
        self.assertTrue(any(canvas.opaque(x, 8) for x in range(0, 6)))
        self.assertTrue(any(canvas.opaque(x, 8) for x in range(14, 20)))
...
class SpecTests(unittest.TestCase):
    ...
    def test_feature_only_composition_loads(self) -> None:
        raw = json.loads(
            (FIXTURES / "bloomling_spec.json").read_text()
        )
        raw["rig"]["eyes"]["blink"] = "none"
        raw["rig"]["composition"] = {
            "mode": "features_only",
            "regions": [[2, 2, 20, 16]],
            "transparent_colors": ["#889966"],
            "transparent_tolerance": 20,
        }
        raw["rig"]["mouth"]["erase"] = False
        path = FIXTURES / "_feature_spec.json"
        path.write_text(json.dumps(raw))
        try:
            rig = load_spec(path).rig
            self.assertEqual(rig.composition_mode, "features_only")
            self.assertEqual(rig.feature_regions, ((2, 2, 20, 16),))
            self.assertEqual(rig.transparent_colors, ((136, 153, 102),))
        finally:
            path.unlink()

    def test_feature_only_mouth_cannot_overlap_eyes(self) -> None:
        raw = json.loads(
            (FIXTURES / "bloomling_spec.json").read_text()
        )
        raw["rig"]["eyes"]["blink"] = "none"
        ...
        raw["rig"]["mouth"]["erase"] = False
        raw["rig"]["mouth"]["box"] = [8, 5, 12, 8]
        path = FIXTURES / "_overlap_spec.json"
        path.write_text(json.dumps(raw))
        try:
            with self.assertRaisesRegex(
                SpecError, "mouth box must begin below"
            ):
                load_spec(path)
        finally:
            path.unlink()

    def test_feature_only_rejects_destructive_mouth_erasure(self) -> None:
        raw = json.loads(
            (FIXTURES / "bloomling_spec.json").read_text()
        )
        raw["rig"]["eyes"]["blink"] = "none"
        ...
        raw["rig"]["mouth"]["erase"] = True
        path = FIXTURES / "_erase_feature_spec.json"
        path.write_text(json.dumps(raw))
        try:
            with self.assertRaisesRegex(
                SpecError, "independent mouth cels"
            ):
                load_spec(path)
        finally:
            path.unlink()
...
class EndToEndTests(unittest.TestCase):
    ...
    def test_stage_target_scales_anchors(self) -> None:
        ...
        self.assertEqual(small["target"]["frame"], [160, 120])
