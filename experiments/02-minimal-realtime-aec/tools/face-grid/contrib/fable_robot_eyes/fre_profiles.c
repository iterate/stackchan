    ...
    fre_eye_draw_t *e)
{
    ...
    /* Brow knit presses the inner upper lid down a little. */
    int32_t knit = -fre_min(rig->brow_tilt_q8[which], 0);
    if (knit > 0 && lid_cut_style) {
        ulid_slope += -mirror * (knit * 700) / 256;
    }
    e->ulid_slope_q12 = ulid_slope;
    e->ulid_bend_q12 = 0;
    e->llid_slope_q12 = 0;
    e->llid_bend_q12 = llid_bend;
    ...
    const fre_face_frame_t *face)
{
    ...
    /* Resting mouths keep a faint smile; speech flattens the arch. */
    int32_t arch = -420 + ((int32_t)kf->mouth_open * 320) / 255;
